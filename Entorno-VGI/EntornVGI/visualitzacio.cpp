...
void Projeccio_Orto(int prj, int w, int h)
{   
...
//			        QUE ES CARREGUEN A LA MATRIU DE PROJECCIÓ GL_PROJECTION
	...
	case 0: // s d PERFIL
		...
		glViewport(w / 2, h / 2, w / 2, h / 2); // Definició del Viewport
		break;
	case 1: // I E PLANTA
		glScissor(0, 0, w / 2, h / 2);
		glViewport(0, 0, w / 2, h / 2); // Definició del Viewport
		break;
	case 2: // s E ALÇAT
		glScissor(0, h/2, w / 2, h / 2);
		glViewport(0, h/2, w / 2, h / 2); // Definició del Viewport
		break;
	case 3: // I d
		glScissor(w/2, 0, w / 2, h / 2);
		glViewport(w/2, 0, w / 2, h / 2); // Definició del Viewport
		break;
	case -1:
		glScissor(0, 0, w, h);
		glViewport(0, 0, w, h);
	...
	int arW = 20 * ((w > h) ? w / (double)h : 1);
	int arH = 20 * ((w > h) ? 1 : h / (double)w);

	// W vs H mantenir l'aspect ratio
	glOrtho(-arW/2, arW/2,-arH/2,arH/2,0,1000); // Matriu de projecció i Volum de Visualització
